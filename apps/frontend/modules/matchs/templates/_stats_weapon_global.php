...
foreach ($kills as $kill) {
    @$players[$kill->getKiller()->getSteamid()][$kill->getWeapon()]["k"]++;
    @$players[$kill->getKilled()->getSteamid()][$kill->getWeapon()]["d"]++;
}
    ...
    <tbody>
        ...
        <?php foreach ($playersData as $playerData): ?>
            ...
            <tr>
                ...
                <?php foreach ($weapons as $weapon): ?>
                    <td style="text-align: center;border-left: 2px solid #DDDDDD; border-right: 1px solid #EEEEEE;" <?php if (@$players[$player->getSteamid()][$weapon]["k"] * 1 == 0) echo 'class="muted"'; ?>>
                        <?php echo @$players[$player->getSteamid()][$weapon]["k"] * 1; ?>
                    </td>
                    <td style="text-align: center;" <?php if (@$players[$player->getSteamid()][$weapon]["d"] * 1 == 0) echo 'class="muted"'; ?>>
                        <?php echo @$players[$player->getSteamid()][$weapon]["d"] * 1; ?>
                    </td>
                    <?php @$weaponsStats[$weapon] += @$players[$player->getSteamid()][$weapon]["k"] * 1; ?>
                <?php endforeach; ?>
            </tr>
        <?php endforeach; ?>
    </tbody>
